...
- **ModalFactory** - all modal opening: `openTaskModal`, `openProjectPicker`, `openTaskPicker`, `openImportModal`, `confirmDialog`, `confirmDuplicateSubtasks`, `promptText`. Never instantiate a modal directly from a view. A project is created and edited on pages of its own (`plugin.router.openProjectCreate` / `openProjectEdit`), not in a modal.
