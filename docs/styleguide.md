...
- **ModalFactory** - all modal opening: `openTaskModal`, `openProjectPicker`, `openTaskPicker`, `openImportModal`, `confirmDialog`, `confirmDuplicateSubtasks`, `promptText`. Never instantiate a modal directly from a view. A project is edited on its own page (`plugin.router.openProjectEdit`), not in a modal.
