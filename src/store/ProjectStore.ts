...
export class ProjectStore implements TaskSource {
    ...
    try {
      ...
      if (file instanceof TFile) {
        ...
        await this.app.vault.process(file, (content) => {
          ...
          return serializeProject(project, this.statusesFor(project), this.refsFor(project, project.filePath))
        })
        this.hydratedBodies.add(project)
      } else {
    ...
    try {
      ...
      if (existing instanceof TFile) {
        ...
        await this.app.vault.process(existing, (content) => {
          ...
          return serializeTask(task, project, parentTask, this.statusesFor(project), this.refsFor(project, filePath))
        })
      } else {
  ...
  async importNoteAsTask(project: Project, file: TFile, opts: ImportNoteOptions): Promise<'imported' | 'skipped'> {
    ...
    const newContent = serializeTask(task, project, null, this.statusesFor(project), this.refsFor(project, newFilePath))
  ...
  async importTaskForest(
    ...
    const writeNode = async (task: Task, parent: Task | null): Promise<void> => {
      ...
      const content = serializeTask(task, project, parent, this.statusesFor(project), this.refsFor(project, dest))
