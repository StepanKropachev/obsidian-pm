...
export class ProjectStore implements TaskSource {
  ...
  /** Status definitions in effect for a project: its own list when defined, else the global one. */
  private statusesFor(project: Project): StatusConfig[] {
    return projectStatuses(project, this.getStatuses())
  }
  ...
  async scheduleAfterChange(project: Project, changedTaskId?: string): Promise<number> {
    const { patches } = computeSchedule(project.tasks, changedTaskId, this.statusesFor(project))
